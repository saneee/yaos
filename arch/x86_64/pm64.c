...
DEFINE_PER_CPU(struct cpu, the_cpu);
...
//DEFINE_PER_CPU(struct exception_stack, exception_stack);
//DEFINE_PER_CPU(struct interrupt_stack, interrupt_stack);
...
static inline void init_ist(cpu_p p)
{
    ...
    for (int v = 1; v < N_EXCEPTION_STACKS + 1; v++) {
        ...
        if (v == DEBUG_STACK) {
            ...
            per_cpu(irq_stack_ptr, cpu) = estacks - 64;
        }
...
{
    cpu_p pcpu = (cpu_p) ((ulong) & the_cpu + (ulong) __per_cpu_load);
